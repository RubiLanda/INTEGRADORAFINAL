...
if año is null and mes is null then

select PEDIDOS.id_pedido as IDPEDIDO, PEDIDOS.f_pedido as FECHAREALIZADA, 
PEDIDOS.f_requerido as FECHAREQUERIDA,
case when PEDIDOS.estado_pedido = 'entregado' then PEDIDOS.f_entrega 
else null
end as FECHAENVIO,
case when PEDIDOS.id_tiendas is not null then TIENDAS.nombre_tienda 
else null
end as TIENDA, 
case when PEDIDOS.id_tiendas is not null then TIENDAS.direccion
else null
end as DIRECCION, 
PEDIDOS.estado_pedido as ESTADO
from PEDIDOS
left join TIENDAS on PEDIDOS.id_tiendas = TIENDAS.id_tienda
where PEDIDOS.id_cliente = idcliente and (PEDIDOS.estado_pedido = 'entregado' or PEDIDOS.estado_pedido = 'cancelado');

end if;

-- el cliente con tienda año elegido  y mes nulo
if año is not null and mes is null then
...
from PEDIDOS
left join TIENDAS on PEDIDOS.id_tiendas = TIENDAS.id_tienda
where PEDIDOS.id_cliente = idcliente and year(PEDIDOS.f_pedido)= año and (PEDIDOS.estado_pedido = 'entregado' or PEDIDOS.estado_pedido = 'cancelado');

end if;

-- el cliente con tienda año nulo y mes elegido
if año is null and mes is not null then
...
from PEDIDOS
left join TIENDAS on PEDIDOS.id_tiendas = TIENDAS.id_tienda
where PEDIDOS.id_cliente = idcliente and month(PEDIDOS.f_pedido)= mes and (PEDIDOS.estado_pedido = 'entregado' or PEDIDOS.estado_pedido = 'cancelado');

end if;

-- el cliente con tienda año elegido y mes elegido
if año is not null and mes is not null then
...
from PEDIDOS
left join TIENDAS on PEDIDOS.id_tiendas = TIENDAS.id_tienda
where PEDIDOS.id_cliente = idcliente and year(PEDIDOS.f_pedido)= año and month(PEDIDOS.f_pedido)= mes and (PEDIDOS.estado_pedido = 'entregado' or PEDIDOS.estado_pedido = 'cancelado');
